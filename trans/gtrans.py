    ...
    def transform(self, X, **transform_params):
        ...
        if isinstance(X.columns, pd.MultiIndex):
            ...
            if pd.__version__ > "0.20.0":
                print("PANDA version > 0.20.0")
                trans.columns = trans.columns.remove_unused_levels()
            else:
...
class GetDataTransformer(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, tickers, cal_ticker=None, start=None, end=None, **params):
        ...
        GetDataProviderTransformer is nearly identical execpt for:
        - arg is "dataProvider" instead of "dataStore"
        - the method call on dataProvider is "get", instead of "combine_data"

        We came to realize that DataStore objects should NOT be used for retrieval (hence, should never have had a combine_data method to retrieve data.
        The correct object ot retrieve data is a DataProvider and the retrieval method is called "get"

        """
        print("DEPRECATED in favor of GetDataProviderTransformer.")
...
class GetDataProviderTransformer(BaseEstimator, TransformerMixin):
    """ 
    A DataFrame transformer that gets raw data (from a DataProvider) of multiple tickers, and assembles into a wide DataFrame
    ...
    dataProvider: a DateProvider object to fetch the data (defaults to GetData(), which is not (yet) a DataProvider oject but implements the correct signatures)
    ...
    def transform(self, X, **transform_params):
        ...
        trans = self.df # pipe_d.fit_transform( self.df )
    ...
    def fit(self, X, y=None, **fit_params):
        ...
        gd = self.gd
        df = gd.get( self.tickers, start=self.start, end=self.end )
